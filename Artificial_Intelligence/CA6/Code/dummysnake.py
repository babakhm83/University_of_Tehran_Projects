...
class DummySnake:
    ...
    def __init__(self, color, pos, file_name=None):
        ...
        self.dirnx = 0
        self.dirny = 1
        self.states_shape = (3, 4, 4, 3, 3, 4, 4, 4, 4, 4, 4, 4)
        try:
    ...
    def move(self, snack, other_snake):
        ...
        new_state[5:7] = self.find_nearest(self.head.pos, other_snake)
        new_state[7:] = self.find_farther(self.head.pos, other_snake, snack)
    ...
    def find_farther(self, self_pos, other, snack):
        r = 0
        l = 0
        u = 0
        d = 0
        ...
        def check_distance(self_pos, body, kind):
            nonlocal r, l, u, d, r_d, l_d, u_d, d_d
            if body.pos[1] == self_pos[1]:
                if body.pos[0] > self_pos[0] and r_d > body.pos[0]:
                    r = kind
                    r_d = body.pos[0]
                if body.pos[0] < self_pos[0] and l_d < body.pos[0]:
                    l = kind
                    l_d = body.pos[0]
            if body.pos[0] == self_pos[0]:
                if body.pos[1] > self_pos[1] and u_d > body.pos[1]:
                    u = kind
                    u_d = body.pos[1]
                if body.pos[1] < self_pos[1] and d_d < body.pos[1]:
                    d = kind
                    d_d = body.pos[1]
        for body in self.body[1:]:
            check_distance(self_pos, body, 1)
        for body in other.body[1:]:
            check_distance(self_pos, body, 1)
        check_distance(self_pos, snack, 2)
        check_distance(self_pos, other.head, 3)
        return r, l, u, d
