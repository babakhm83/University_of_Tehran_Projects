...
class Snake:
    ...
    def __init__(self, color, pos, file_name=None):
        ...
        self.dirnx = 0
        self.dirny = 1
        self.states_shape = (2,4, 4, 4,4,4,4, 4)
        try:
        ...
        self.lr = 0.01  # TODO: Learning rate
        self.discount_factor = 0.8  # TODO: Discount factor
    ...
    def move(self, snack, other_snake):
        ...
        new_state[0] = 1 if len(self.body) > len(other_snake.body) else 0
        ...
        if other_snake.head.pos[1] > self.head.pos[1]:
            new_state[2] += 1
        new_state[3:] = self.find_nearest(self.head.pos, other_snake)
        ...
        for body in other.body[1:]:
            if body.pos[1] == self_pos[1]:
                if body.pos[0] > self_pos[0]:
                    r = min(r, body.pos[0]-self_pos[0])
                else:
                    l = min(l, self_pos[0]-body.pos[0])
            if body.pos[0] == self_pos[0]:
                if body.pos[1] > self_pos[1]:
                    u = min(u, body.pos[1]-self_pos[1])
                else:
                    d = min(d, self_pos[1]-body.pos[1])
        return l//5, r//5, u//5, d//5
